...
def load_cases(samples_dir: Path) -> list[dict]:
    """
    Scan samples_dir for batch_*/case_* subdirectories.
    Each case has one or more product NPZ files.
    Returns list of dicts sorted by date.
    """
    cases = []
    for case_dir in sorted(samples_dir.glob("batch_*_case_*")):
        npz_files = list(case_dir.glob("*.npz"))
        if not npz_files:
            continue

        # Read date from json sidecar if available, else parse from dirname
        date_str = _infer_date(case_dir)
    ...
    print(f"Found {len(cases)} case(s) in '{samples_dir}'")
    return cases


def _infer_date(case_dir: Path) -> str:
    ...
    for json_path in case_dir.glob("*.json"):
        try:
            ...
            pass
    # Fallback: parse from directory name (batch_0000_case_00 won't have date)
...
def infer_dim_names(shape: tuple, array_name: str) -> list[str]:
    """
    Assign dimension names based on array shape and name.
    STRIDE arrays follow these conventions:
      (batch/ensemble, channels, H, W)  — 4D
      (channels, H, W)                  — 3D
      (channels,)                       — 1D/2D
    After stacking along time the leading dim becomes 'time'.
    """
    ndim = len(shape)
    if ndim == 4:
        # (time, ensemble_or_channel, H, W)
        if "member" in array_name or "ensemble" in array_name or shape[1] > 4:
            return ["time", "member", "y", "x"]
        return ["time", "channel", "y", "x"]
    if ndim == 3:
        return ["time", "y", "x"]
    if ndim == 2:
        return ["time", "channel"]
