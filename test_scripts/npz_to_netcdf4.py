...
def _is_date_dir(d: Path) -> bool:
    ...
    return d.is_dir() and d.name.isdigit() and len(d.name) == 10 #10 #8
    ...
    for product_name in products_to_write:
        ...
        for arr_name in sorted(array_names):
            ...
            for c in cases:
                ...
                if arr is None:
                    print(f"  WARNING: '{arr_name}' missing in case {c['case_id']}, skipping.")
                    slices = []
                    break
                # Normalise to always have shape (1, ...) so concatenation
                # along axis=0 always produces (time, ...).
                # - (1, channels, H, W)  -> keep as-is          -> (1, C, H, W)
                # - (16, 1, H, W)        -> wrap in time axis   -> (1, 16, 1, H, W)  <- avoid
                # Rule: if leading dim == 1 it is already the batch/time dim; else wrap.
                if arr.ndim > 0 and arr.shape[0] != 1:
                    arr = arr[np.newaxis]   # (members, C, H, W) -> (1, members, C, H, W)
                slices.append(arr)

            if slices:
                stacked[arr_name] = np.concatenate(slices, axis=0)
                print(f"  stacked '{arr_name}': per-case={slices[0].shape} x {len(slices)} -> {stacked[arr_name].shape}")
